...
/**
 * This script holds the Team_Games_Widget class.
...
class Team_Games_Widget extends Game_Widget{
    ...
    public function widget($args, $instance){
        ...
        if(!$teams == ''){
			...
			/** @see /src/extract_transform.php function extract_transform() */
			$conditions[] = db_prepare("origin_team IN (" . $placeholders . ")", ...$teams);
        }
        // select
		...
		if($link !== ''){
			...
			if($teams !== '' && strpos($teams, ',') === false && $select !== self::BOTH){
				...
				$link_url = get_team_link($teams, $field);
			}else{
